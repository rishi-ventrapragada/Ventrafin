  ...
  <div class="flex max-w-3xl flex-col gap-3">
    ...
    <section class="card divide-y divide-slate-100">
      <div class="flex items-center gap-3 p-4">
        <AppIcon name="person" :size="22" class="text-slate-500" />
        <div class="mr-auto">
          <div class="font-medium">Signed in with Google</div>
          <div class="text-sm text-slate-500">{{ app.auth.user.value?.email }}</div>
        </div>
        <Button label="Sign out" severity="secondary" outlined @click="signOut">
          <template #icon><AppIcon name="logout" :size="17" /></template>
        </Button>
      </div>
      <div class="flex items-center gap-3 p-4">
        <AppIcon name="sync" :size="22" class="text-slate-500" />
        <div>
          <div class="font-medium">Sync with the phone</div>
          <div class="text-sm text-slate-500">{{ app.online.value ? live : 'Offline.' }}</div>
        </div>
      </div>
      <div class="flex items-center gap-3 p-4">
        <AppIcon name="palette" :size="22" class="text-slate-500" />
        <div>
          <div class="font-medium">Theme</div>
          <div class="text-sm text-slate-500">Ocean. More themes are coming in a later update.</div>
        </div>
      </div>
      <div class="flex items-center gap-3 p-4">
        <AppIcon name="lock" :size="22" class="text-slate-500" />
        <div>
