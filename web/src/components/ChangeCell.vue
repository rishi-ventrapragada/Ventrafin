...
const view = computed(() => {
  ...
  if (diff === 0) return { text: 'same', color: 'text-slate-600', icon: null }
...
<template>
  <span class="inline-flex items-center justify-end gap-0.5 font-semibold whitespace-nowrap tabular-nums" :class="view.color">
    <AppIcon v-if="view.icon" :name="view.icon" :size="16" />{{ view.text }}
    <span v-if="'pct' in view" class="ml-1 text-[0.9em] font-normal text-slate-600">{{ view.pct }}%</span>
  </span>
</template>
