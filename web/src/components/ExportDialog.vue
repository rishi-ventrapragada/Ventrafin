...
// exactly what's on screen, filters included.
...
<template>
  <Dialog v-model:visible="visible" modal header="Download transactions (CSV for Excel)" :style="{ width: 'min(40rem, 96vw)' }">
    <div class="flex flex-col gap-3">
      <div role="radiogroup" aria-label="Which transactions" class="flex flex-col gap-1" data-testid="export-ranges">
        <label
          ...
          :data-range="o.value"
        >
          <input v-model="choice" type="radio" name="export-range" :value="o.value" class="mt-1 accent-[var(--p-primary-color)]" />
          <span>
      ...
      <div v-if="choice === 'custom'" class="flex flex-wrap items-center gap-2">
        ...
        <input id="export-from" v-model="customFrom" type="date" class="rounded-md border border-slate-300 px-2 py-1" data-testid="export-from" />
        <label class="text-sm font-medium" for="export-to">to</label>
        <input id="export-to" v-model="customTo" type="date" class="rounded-md border border-slate-300 px-2 py-1" data-testid="export-to" />
    ...
    <template #footer>
      ...
      <Button label="Download CSV" :loading="busy" :disabled="problem !== null" data-testid="export-download" @click="download">
