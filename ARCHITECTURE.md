...
- **Reporting aggregates** are SQL functions called via `rpc()` from both apps. They are SECURITY INVOKER with no owner parameter, so a caller can only ever aggregate their own rows:
  ...
  - `get_account_totals(p_month)`: one row per account (archived included) with that month's spending, income, transfers out and in, and entry count. The web Accounts page shows it.
...
- **Bills** (`DECISIONS.md` D23): every month has one instance of a bill, due on `due_day` clamped to the month's last day (`private.bill_due_date`).
  - `get_bill_schedule(p_today default null)`: each bill with `next_due_date` (the month after `paid_through_month`), `days_until` (negative = overdue), `status` (`overdue` / `due_today` / `due_soon` within 7 days / `upcoming`) `overdue_count` and `upcoming_due_dates` (the due dates of the next three unpaid months, already clamped). Both apps show it; the phone schedules reminders from `upcoming_due_dates`, so the clamping rule exists only in SQL.
